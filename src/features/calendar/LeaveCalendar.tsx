...
import { Calendar as CalendarIcon, ChevronLeft, ChevronRight, Flag, Info, Moon, Sun, User } from 'lucide-react';
...
export function LeaveCalendar() {
  ...
  const selectedDateRequests = requests?.filter((request) => {
    ...
    const isHoliday = holidays?.some((holiday) => isSameDay(parseISO(holiday.date), selectedDate));
  ...
  const selectedDayHoliday = holidays?.find((holiday) => isSameDay(parseISO(holiday.date), selectedDate));
    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-black">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 bg-white rounded-2xl border border-slate-200 p-1.5 shadow-sm self-start md:self-center">
          <button type="button" onClick={prevMonth} className="p-2.5 hover:bg-slate-50 rounded-xl transition-all text-slate-400 hover:text-indigo-600">
            <ChevronLeft className="w-5 h-5" />
          </button>
          <span className="text-base font-bold min-w-[140px] text-center text-slate-700">
            {format(currentDate, 'yyyy年 M月', { locale: ja })}
          </span>
          <button type="button" onClick={nextMonth} className="p-2.5 hover:bg-slate-50 rounded-xl transition-all text-slate-400 hover:text-indigo-600">
          ...
          <CardContent className="p-0">
              ...
              {['日', '月', '火', '水', '木', '金', '土'].map((day, index) => (
                <div
                  ...
                  className={`py-4 text-center text-[11px] font-black uppercase tracking-widest ${
                    index === 0 ? 'text-red-500 bg-red-50/30' : index === 6 ? 'text-blue-500 bg-blue-50/30' : 'text-slate-400'
                  }`}
                >
                  {day}
                </div>
            ...
            <div className="grid grid-cols-7">
              {days.map((day) => {
                ...
                const holiday = holidays?.find((item) => isSameDay(parseISO(item.date), day));
                  ...
                  >
                    ...
                    <div className="space-y-1 overflow-hidden">
                      {dayRequests.slice(0, 3).map((request) => (
                        <div
                          key={request.id}
                          className={`text-[9px] md:text-[10px] px-1.5 md:px-2 py-0.5 rounded-lg border font-bold truncate flex items-center ${typeLightColors[request.type] || typeLightColors.paid_leave}`}
                        >
                          {request.is_half_day && (
                            request.half_day_type === 'AM' ? <Sun className="w-2 h-2 mr-1 text-amber-500" /> : <Moon className="w-2 h-2 mr-1 text-indigo-400" />
                          )}
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-xl shadow-slate-200/50 rounded-[2rem] bg-white overflow-hidden">
            <CardHeader className="bg-slate-900 text-white py-6 px-8 border-none">
              <div className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">選択した日付</div>
              ...
              <div className="space-y-4">
                {selectedDateRequests.length > 0 ? (
                  selectedDateRequests.map((request) => (
                    <div key={request.id} className="flex items-start space-x-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:bg-white transition-all group">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg ${typeColors[request.type] || typeColors.paid_leave}`}>
                        {request.is_half_day ? (
                          request.half_day_type === 'AM' ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />
                        ) : (
                        ...
                        <div className="text-xs font-black text-indigo-600 uppercase mt-0.5 flex items-center">
                          ...
                          {request.is_half_day && (
                            <span className="ml-2 bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded text-[9px]">
                              {request.half_day_type === 'AM' ? '午前半休' : '午後半休'}
                            </span>
                          )}
                        </div>
                        {request.reason && (
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(typeLabels).map(([type, label]) => (
                <div key={type} className="flex items-center space-x-2.5 p-2 rounded-xl border border-slate-50 bg-slate-50/30">
                  <span className={`w-2.5 h-2.5 rounded-full shadow-sm ${typeColors[type]}`} />
