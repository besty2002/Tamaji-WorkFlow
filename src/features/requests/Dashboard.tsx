...
export function Dashboard() {
  ...
  if (balanceError || requestsError || grantsError) {
    const errorMsg = (balanceError as any)?.message || (requestsError as any)?.message || (grantsError as any)?.message || 'ダッシュボードデータの読み込み中にエラーが発生しました。';
        ...
        <Card>
          ...
          <CardContent className="p-0">
            {recentRequests && recentRequests.length > 0 ? (
                  ...
                  <div key={req.id} className="p-4 flex justify-between items-center hover:bg-gray-50 transition-colors">
                    <div>
                      <div className="font-medium text-gray-900 text-sm">
                        ...
                        {req.start_date !== req.end_date && ` - ${format(new Date(req.end_date), 'yyyy/MM/dd', { locale: ja })}`}
                      </div>
                      <div className="text-xs text-gray-500">{typeLabels[req.type]} {req.is_half_day && `(${req.half_day_type === 'AM' ? '午前' : '午後'}半休)`}</div>
                    </div>
                    <div className="flex items-center space-x-3">
                      ...
                      <Link to={`/requests/${req.id}`} className="text-indigo-600 hover:text-indigo-900 text-xs font-medium">
                        表示
                      </Link>
                    </div>
                  </div>
              ...
              <div className="p-8 text-center text-gray-500 text-sm">最近の申請はありません。</div>
            )}
          </CardContent>
        </Card>

        <Card>
          <CardHeader>休暇付与履歴</CardHeader>
          <CardContent className="p-0">
            {grantHistory && grantHistory.length > 0 ? (
              <div className="divide-y divide-gray-200">
                {grantHistory.map(grant => (
                  <div key={grant.id} className="p-4 flex justify-between items-center">
                    <div>
                      <div className="font-medium text-gray-900 text-sm">
                        {format(new Date(grant.granted_at), 'yyyy/MM/dd', { locale: ja })}
                      </div>
                      <div className="text-xs text-gray-500">{grant.reason || '定期付与'}</div>
                    </div>
                    <div className="text-lg font-bold text-green-600">
                      +{grant.days} <span className="text-xs font-normal text-gray-500">日</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="p-8 text-center text-gray-500 text-sm">付与履歴가ありません。</div>
            )}
          </CardContent>
        </Card>
      </div>
